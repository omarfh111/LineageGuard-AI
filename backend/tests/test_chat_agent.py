...
from app.services.chat_agent import HybridChatAgent, _propose_action
...
class FakeCompletion:
    async def answer(self, question: str, sources: list[RagCitation]) -> str:
        ...
        return "orders_dashboard is a live DataHub match."
...
class AnyCompletion:
    async def answer(self, question: str, sources: list[RagCitation]) -> str:
        return "Grounded metadata answer."


@pytest.mark.asyncio
async def test_hybrid_chat_retrieves_then_verifies_with_datahub_mcp() -> None:
    response = await HybridChatAgent(
        FakeDataHub(), FakeIndex(), completion=FakeCompletion()  # type: ignore[arg-type]
    ).respond(ChatRequest(message="Which assets depend on orders?"))

    assert response.answer == "orders_dashboard is a live DataHub match."
    assert len(response.citations) == 2
    assert response.citations[1].source == "datahub_mcp_live"
...
async def test_agentic_router_invokes_schema_tool_only_for_schema_question() -> None:
    ...
    response = await HybridChatAgent(
        datahub, SchemaIndex(), completion=AnyCompletion()  # type: ignore[arg-type]
